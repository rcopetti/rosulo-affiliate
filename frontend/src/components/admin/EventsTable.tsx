...
export function EventsTable({ events }: { events: Event[] }) {
  return (
    <Table className="w-full table-fixed">
      <TableHead>
        <TableRow>
          <TableHeader className="w-44">Event ID</TableHeader>
          <TableHeader className="w-44">Time</TableHeader>
          ...
          <TableHeader className="w-28 text-right">Amount</TableHeader>
        </TableRow>
      </TableHead>
      <TableBody>
        {events.map((e) => (
          <TableRow key={e.id}>
            ...
            <TableCell className="whitespace-nowrap text-xs">{formatDateTime(e.occurred_at)}</TableCell>
